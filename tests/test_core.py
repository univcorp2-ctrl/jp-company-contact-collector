...
def company() -> Company:
    return Company("123", "Example株式会社", 2_000_000_000, "2025", "https://example.co.jp")


def test_role_email_policy() -> None:
    assert is_role_email("info@example.co.jp", "https://example.co.jp")
    assert is_role_email("sales-team@example.co.jp", "https://example.co.jp")
    assert not is_role_email("taro.yamada@example.co.jp", "https://example.co.jp")
    assert not is_role_email("info@gmail.com", "https://example.co.jp")
    assert not is_role_email("info@other.co.jp", "https://example.co.jp")
...
def test_extract_email_form_and_same_domain_links() -> None:
    ...
    <html><head><title>お問い合わせ</title></head><body>
      ...
      <a href="mailto:taro.yamada@example.co.jp">personal</a>
    ...
    """
    contacts, links = extract_contacts(html, "https://example.co.jp/contact", company())
    values = {(item.contact_type, item.contact_value) for item in contacts}
    assert ("email", "info@example.co.jp") in values
    assert ("inquiry_form", "https://example.co.jp/contact/send") in values
    assert ("contact_page", "https://example.co.jp/company/contact") in values
...
def test_revenue_filter_and_dedupe(tmp_path: Path) -> None:
    ...
    source.write_text(
        ...
        "1,A社,2000,2025,https://a.example.jp\n"
        "1,A社重複,3000,2025,https://a.example.jp\n"
        "2,B社,100,2025,https://b.example.jp\n",
        encoding="utf-8",
    )
...
def test_csv_is_utf8_sig(tmp_path: Path) -> None:
    html = '<html><title>Contact</title><a href="mailto:info@example.co.jp">mail</a></html>'
    contacts, _ = extract_contacts(html, "https://example.co.jp/contact", company())
